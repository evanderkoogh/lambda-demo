...
import { logger, NotFoundError, initTracer, trace, SpanStatusCode } from '@frontend-demo/shared';
...
export async function handler(event: BackendRequest): Promise<BackendResponse> {
  ...
  // ADOT's Lambda instrumentation extracts W3C traceparent from event.headers
  // (populated by middleware) and parents frontend-demo-backend correctly.
